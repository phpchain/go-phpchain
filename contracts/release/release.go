...
type Config struct {
	Oracle common.Address // PHPChain address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the PHPChain service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var phpchain *eth.PHPChain
	if err := ctx.Service(&phpchain); err == nil {
		apiBackend = phpchain.ApiBackend
	} else {
		var phpchain *les.LightPHPChain
		if err := ctx.Service(&phpchain); err == nil {
			apiBackend = phpchain.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/phpchain/go-phpchain/releases for new releases")
