...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/phpchain                                                            && \
	(cd $GOPATH/src/github.com/phpchain && git clone --depth=1 https://github.com/phpchain/go-phpchain) && \
  go build -v github.com/phpchain/go-phpchain/cmd/faucet                                              && \
