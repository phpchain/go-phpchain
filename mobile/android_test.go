...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.phpchain", new NodeConfig());
			node.start();
			...
			PHPChainClient ec = node.getPHPChainClient();
	...
	// signatures too. Regression test for go-phpchain issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/phpchain/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.phpchain", "github.com/phpchain/go-phpchain/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "phpchain", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
