...
import (
	...
	"github.com/phpchain/go-phpchain/consensus/misc"
	...
	"github.com/phpchain/go-phpchain/eth/fetcher"
...
// NewProtocolManager returns a new phpchain sub protocol manager. The PHPChain sub protocol manages peers capable
// with the phpchain network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing PHPChain peer", "peer", id)

	// Unregister the peer from the downloader and PHPChain peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping PHPChain protocol")
	...
	log.Info("PHPChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("PHPChain peer connected", "name", p.Name())

	// Execute the PHPChain handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("PHPChain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("PHPChain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("PHPChain message handling failed", "err", err)
			return err
		}
	}
}
...
// EthNodeInfo represents a short summary of the PHPChain sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    uint64      `json:"network"`    // PHPChain network ID (1=Frontier, 2=Morden, Ropsten=3)
