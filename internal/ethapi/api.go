...
// PublicPHPChainAPI provides an API to access PHPChain related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicPHPChainAPI struct {
	b Backend
}

// NewPublicPHPChainAPI creates a new PHPChain protocol API.
func NewPublicPHPChainAPI(b Backend) *PublicPHPChainAPI {
	return &PublicPHPChainAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicPHPChainAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current PHPChain protocol version this node supports
func (s *PublicPHPChainAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicPHPChainAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19PHPChain Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19PHPChain Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an PHPChain ECDSA signature for:
// keccack256("\x19PHPChain Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/phpchain/go-phpchain/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19PHPChain Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/phpchain/go-phpchain/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid PHPChain signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the PHPChain blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new PHPChain blockchain API.
...
//
// https://github.com/phpchain/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of PHPChain APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of PHPChain APIs exposed over the private
// debugging endpoint.
...
// Version returns the current phpchain protocol version.
