...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light PHPChain protocol")
	...
	log.Info("Light PHPChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light PHPChain peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light PHPChain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light PHPChain message arrived", "code", msg.Code, "bytes", msg.Size)
